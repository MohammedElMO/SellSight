...
public class RestoreUserUseCase {
    ...
    public void execute(String userId) {
        ...
        if (user.getRole() == Role.SUPER_ADMIN) {
            throw new SuperAdminProtectedException("restore");
        }
        if (user.getRole() == Role.ADMIN) {
            throw new IllegalStateException("Admin accounts cannot be managed via this endpoint. Use the Super Admin panel.");
        }
        user.restore();
